...
async function main() {
  ...
  for (const file of files) {
    ...
    console.log(`Converted "${metadata.title}" → ${OUTPUT_DIR}/${slug}.md`);
    ...
    newPosts.push({ slug, metadata, canonicalUrl });
  }

  if (newPosts.length === 0) {
    console.log('No new posts to convert.');
    return;
  }
  ...
  if (summaryFile) {
    let summary = '';
    for (const { slug, metadata, canonicalUrl } of newPosts) {
      const hashtags = metadata.tags
        .map(t => `#${t.replace(/\s+/g, '')}`)
        .join(' ');

      summary += `## Converted: ${metadata.title}\n\n`;
      summary += `**Converted Markdown:** download from the \`converted-markdown\` artifact\n\n`;
